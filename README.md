...
- [2. Usage](#2-usage)
  ...
  - [2.2 Suppress lints](#22-suppress-lints)
...
// Suppress specific lint messages
let mut suppress_lints = HashSet::new();
...
The function `process_source_file` is the main API provided by ANCPP. It takes a `FileProvider`, a `HeaderFileCache`, a map of predefined macros, compile features, and suppress lints. It returns the result as a `Result<PreprocessResult, PreprocessorError>`. A `PreprocessResult` contains a vector of `TokenWithLocation` (the preprocessed tokens, ready for consumption by the parser of compilers) and a vector of `Lint` (warnings and hints generated during preprocessing).
...
ANCPP provides a built-in linter to report potential issues and suggestions during preprocessing. Each lint message has a unique identifier, and you can suppress specific lint by adding their identifiers to the `suppress_lints` set.

- `directive_warn` (warn): Message that generated by the directive `warn`.
- `include_guard_absent` (warn): A header file does not have `#pragma once` or include guard.
- `traditional_include_guard` (info): A header file has an include guard, but it is recommended to use `#pragma once` for better practice.
- `unconventional_include_guard` (info): The include guard macro name does not follow the convention of being in uppercase and ending with `FILE_NAME_H`.
- `macro_invocation_single_argument_multiple_tokens` (info): A macro invocation argument contains multiple tokens, which may lead to unexpected behaviors.
- `identical_redefinition` (info): A macro is redefined with an identical replacement, which is redundant and may indicate a mistake.
- `undef_nonexistent_macro` (warn): A macro that has not been defined is undefined using `#undef`, which may indicate a mistake.
...
[ANCPP RESTRICTION]: Most preprocessors allow redefine directly and simply issue a warning, but this can easily lead to mistakes.
...
You can redefine a macro if the new replacement is identical, but a lint message will still be generated to indicate the redundant redefinition.

```c
#define BUZZ 789
#define BUZZ 789                // Info: Identical Redefinition
```
...
By default ANCC pass the `ancpp::consts::C23_KEYWORD_STRS` constant to reserve all C23 keywords, this helps avoid accidental macro definitions that conflict with C language keywords.
...
If the identifier does not have an associated macro, the processor will generate a warning message.

```c
#undef FOO                      // Warning: Macro "FOO" has not been defined.
```

While traditional preprocessors silently ignore attempts to undefine undefined macros, ANCPP treats it as a warning. This lint helps catch typos.
...
**Redefinition of function-like macros**

You can not redefine an existing function-like macro with a new definition directly, you must first undefine it using the `#undef` directive.

```c
#define ADD(x, y) (x) + (y)
#define ADD(x) (x) + 1          // Error: Redefinition an existing macro

#undef ADD
#define ADD(x) (x) + 1          // Now it is ok
```

Identical redefinition of function-like macros is also allowed, though a lint message will be generated to indicate the redundant redefinition.

```c
#define ADD(x, y) (x) + (y)
#define ADD(left, right) (left) + (right)   // Info: Identical Redefinition
```
